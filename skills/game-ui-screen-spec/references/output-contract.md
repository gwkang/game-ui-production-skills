...
When a project already has an approved shared-capture map, retain its capture-group IDs and require a unique observation key formed from the group ID plus the declared component/consumer and state. Shared groups must retain the same target and fixture; each observation still needs its own evidence verdict. Do not reinterpret an unapproved map or make grouped IDs the default for new specifications.
