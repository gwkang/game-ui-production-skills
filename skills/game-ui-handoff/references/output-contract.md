...
Copy the upstream `binding`, catalog artifact fingerprint, consumer, allowed instance inputs, protected-property names and locators verbatim. Map the mockup result cell to mockup fidelity unchanged. Populate required states from the exact approved catalog state IDs for this binding and fingerprint; the mockup represented-states cell is coverage evidence, not a required-states cell. Reject missing or unrepresented required states, and keep representation-status annotations out of state IDs. Do not remove the `reuse:` prefix, normalize identifiers, or substitute an implementation helper name for the binding. A `BLOCKED` packet still emits the heading, header, and every complete binding row before the recovery handoff; a route-only response is incomplete.
...
Emit all ten numbered slots in this order for every packet, including `BLOCKED` packets. When evidence is unavailable, keep the slot and state `OPEN` or `BLOCKED` with its missing source; do not collapse the packet to the component matrix and route labels.
